...
use lancedb::{Connection, Table, query::{QueryBase, ExecutableQuery}};
...
fn extract_record_columns(batch: &RecordBatch) -> Result<RecordColumns> {
    ...
    let overview_col = batch.column_by_name("overview")
    ...
    let topic_array = topic_col.as_any().downcast_ref::<StringArray>()
    ...
    let overview_array = overview_col.as_any().downcast_ref::<StringArray>()
...
fn display_single_record(columns: &RecordColumns, index: usize, record_count: usize) -> Result<()> {
    ...
    let overview_preview = columns.overview_array.value(index)
        .chars()
        .take(50)
        .collect::<String>();
