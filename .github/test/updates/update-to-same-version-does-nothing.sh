...
INSTALLED_VERSION=$("$HOME/.cargo/bin/blizz" --version | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -n1)
[ -n "${INSTALLED_VERSION:-}" ] || fail "Could not determine installed blizz version"

# Sanity: no snapshots directory yet
[ ! -d "$HOME/.blizz/snapshots" ] || fail "Snapshots directory should not exist before update"

# Case 1: update to same version with v-prefix
OUT1=$("$HOME/.cargo/bin/blizz" update --version "v$INSTALLED_VERSION" 2>&1 || true)
...
OUT2=$("$HOME/.cargo/bin/blizz" update --version "$INSTALLED_VERSION" 2>&1 || true)
...
[ ! -d "$HOME/.blizz/snapshots" ] || fail "No-op update should not create snapshots"

# Ensure binary still works after no-op updates
"$HOME/.cargo/bin/blizz" --help >/dev/null || fail "blizz should still work after no-op update"
