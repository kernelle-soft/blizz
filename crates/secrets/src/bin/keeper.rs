...
mod tests {
  ...
  fn test_create_new_vault_store_with_trimmed_password() {
    ...
    let empty_credentials = HashMap::new();
    let store =
      PasswordBasedCredentialStore::new(&empty_credentials, password_with_whitespace.trim())
        .unwrap();
    ...
    assert!(
      verify_untrimmed.is_ok(),
      "Should also verify with untrimmed password because verify_password trims it"
    );
  ...
  fn test_create_new_vault_directory_creation_edge_case() {
    ...
    let result =
      create_new_vault_non_interactive(&cred_path, "existing_dir_test", "existing_dir_test");
    assert!(result.is_ok());
    assert!(cred_path.exists());
  }
  ...
  fn test_create_new_vault_success_logging() {
    ...
    // Success logging verified by execution completing without panic
  }
  ...
  fn test_real_create_new_vault_function_whitespace_trimming() {
    ...
    set_test_prompt_responses(vec![
      "  spaced_password  ".to_string(),
      "  spaced_password  ".to_string(),
    ]);
