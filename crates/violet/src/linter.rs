...
use crate::{
  config::Config,
  metrics::{
    analyze_no_duh_comments, calculate_file_metrics, calculate_function_metrics, get_function_name,
...
impl Linter {
  ...
  fn check_function_violations(
    ...
    // Check for no-duh comments
    if !has_ignore_directive(source_code, metrics.start_line, "no-duh-comments") {
      let no_duh_lines = analyze_no_duh_comments(function_node, source_code, language);
      if no_duh_lines.len() > rules.max_no_duh_comments {
        violations.push(Violation {
                    rule: "no-duh-comments".to_string(),
                    severity: Severity::Warning,
                    message: format!(
                        "Function '{}' has too many obvious comments: {} (max: {}). Remove comments that just repeat the code.",
                        function_name, no_duh_lines.len(), rules.max_no_duh_comments
                    ),
                    file: file_path.to_path_buf(),
                    line: metrics.start_line,
                    column: None,
                    suggestion: Some("Remove comments that simply restate what the code does".to_string()),
                });
      } else {
        // Create individual violations for each no-duh comment
        for &line_num in &no_duh_lines {
          violations.push(Violation {
            rule: "no-duh-comment".to_string(),
            severity: Severity::Info,
            message: "This comment appears to simply restate what the code does".to_string(),
            file: file_path.to_path_buf(),
            line: line_num,
            column: None,
            suggestion: Some(
              "Consider removing obvious comments or adding meaningful context".to_string(),
            ),
          });
        }
      }
...
mod tests {
  ...
  fn lint_code(code: &str, language: Language, config: &Config) -> Vec<Violation> {
    ...
    violations.extend(linter.check_file_violations(
      Path::new("test.js"),
      &file_metrics,
      code,
      language,
    ...
    for function_node in functions {
      let function_metrics =
        crate::metrics::calculate_function_metrics(function_node, code, language);
  ...
  fn test_function_length_violation() {
    ...
    let length_violations: Vec<_> =
      violations.iter().filter(|v| v.rule == "function-length").collect();
  ...
  fn test_function_depth_violation() {
    ...
    let depth_violations: Vec<_> =
      violations.iter().filter(|v| v.rule == "function-depth").collect();
  ...
  fn test_complexity_violation() {
    ...
    let complexity_violations: Vec<_> =
      violations.iter().filter(|v| v.rule == "complexity").collect();
  ...
  fn test_file_length_violation() {
    ...
    let file_violations: Vec<_> = violations.iter().filter(|v| v.rule == "file-length").collect();
  ...
  fn test_file_depth_violation() {
    ...
    let depth_violations: Vec<_> = violations.iter().filter(|v| v.rule == "file-depth").collect();
  ...
  fn test_ignore_all_directive() {
    ...
    let function_violations: Vec<_> = violations
      .iter()
      .filter(|v| {
        matches!(
          v.rule.as_str(),
          "max-params" | "function-length" | "function-depth" | "complexity"
        )
      })
      .collect();
  ...
  fn test_typescript_parsing() {
    ...
    let code =
      "function test(a: number, b: number, c: number, d: number): number { return a + b + c + d; }";
    let config = create_test_config();
  ...
  fn test_violation_message_content() {
    ...
    let param_violation =
      violations.iter().find(|v| v.rule == "max-params").expect("Should have max-params violation");
...
"#;
    ...
    let add_violations: Vec<_> = violations.iter().filter(|v| v.message.contains("add")).collect();
...
"#;
  ...
  #[test]
  fn test_no_duh_comments_integration() {
    let config = create_test_config();
    let code_with_no_duh_comments = r#"
function processUserData(userData) {
    ...
    // This comment explains complex business logic for premium calculations
...
"#;
    let violations = lint_code(code_with_no_duh_comments, Language::JavaScript, &config);

    // Should find no-duh comment violations
    let no_duh_violations: Vec<_> = violations
      .iter()
      .filter(|v| v.rule == "no-duh-comment" || v.rule == "no-duh-comments")
      .collect();

    assert!(!no_duh_violations.is_empty(), "Should detect no-duh comments");

    // Check that the good comment is not flagged
    let line_5_violations: Vec<_> = violations
      .iter()
      .filter(|v| v.line == 8) // The line with the good comment
      .collect();
    assert_eq!(line_5_violations.len(), 0, "Good comments should not be flagged as no-duh");
  }

  #[test]
  fn test_no_duh_comments_threshold() {
    let mut config = create_test_config();
    config.rules.max_no_duh_comments = 1; // Set low threshold

    let code_with_many_no_duh_comments = r#"
...
"#;
    let violations = lint_code(code_with_many_no_duh_comments, Language::JavaScript, &config);

    // Should find threshold violation for too many no-duh comments
    let threshold_violations: Vec<_> =
      violations.iter().filter(|v| v.rule == "no-duh-comments").collect();

    assert!(
      !threshold_violations.is_empty(),
      "Should detect threshold violation for too many no-duh comments"
    );
  }

  #[test]
  fn test_no_duh_comments_ignore_directive() {
    let config = create_test_config();
    let code_with_ignored_comments = r#"
// violet-ignore no-duh-comments
function ignoredFunction(userData) {
    // Set the user variable
    const user = userData;
...
"#;
    let violations = lint_code(code_with_ignored_comments, Language::JavaScript, &config);

    // Should not find any no-duh comment violations due to ignore directive
    let no_duh_violations: Vec<_> = violations
      .iter()
      .filter(|v| v.rule == "no-duh-comment" || v.rule == "no-duh-comments")
      .collect();
    ...
    assert_eq!(
      no_duh_violations.len(),
      0,
      "Should ignore no-duh comments when directive is present"
    );
  }
}
