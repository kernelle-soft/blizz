...
use config::Config;
use linter::{format_violations, Linter};
...
pub fn run_cli(cli: Cli) -> Result<()> {
  ...
  match cli.command {
    Some(Commands::Init { output, force }) => init_config(output, force),
    Some(Commands::Config) => show_config(cli.config),
    Some(Commands::Languages) => {
      show_languages();
      Ok(())
    }
    None => analyze_code(cli),
  }
}
...
fn show_languages() {
  ...
  for (name, extensions) in &languages {
    println!("  {} {}", name.green().bold(), format!("({})", extensions).dimmed());
  }
  println!();
...
fn analyze_code(cli: Cli) -> Result<()> {
  if cli.paths.is_empty() {
    println!("{} No paths specified, analyzing current directory...", "ℹ️".blue());
...
fn analyze_paths(paths: &[PathBuf], cli: Cli) -> Result<()> {
  ...
  if cli.verbose {
    println!("{} Analyzing {} path(s)...", "🔍".blue(), paths.len());
  }
  ...
  if !all_violations.is_empty() {
    let error_count =
      all_violations.iter().filter(|v| matches!(v.severity, Severity::Error)).count();
...
mod cli_tests {
  ...
  fn test_run_cli_init_command() {
    ...
    let cli = Cli {
      command: Some(Commands::Init { output: config_path.clone(), force: false }),
      paths: vec![],
      config: None,
      format: OutputFormat::Pretty,
      errors_only: false,
