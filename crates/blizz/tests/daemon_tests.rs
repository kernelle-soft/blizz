use anyhow::Result;
use blizz::daemon::{EmbeddingService, EmbeddingRequest, EmbeddingResponse};
...
mod daemon_tests {
    ...
    async fn test_handle_request_with_model_failure() {
        let model = MockEmbeddingModel::new()
            .with_failure_on("failing text".to_string());
        
        let mut service = EmbeddingService::new(model);
        
        let request = EmbeddingRequest {
            texts: vec!["failing text".to_string()],
    ...
    async fn test_handle_request_large_batch() {
        let model = MockEmbeddingModel::new()
            .with_embeddings(vec![vec![1.0, 2.0]; 50]); // 50 identical embeddings
    ...
    async fn test_service_handles_unicode_text() {
        let model = MockEmbeddingModel::new()
            .with_embeddings(vec![vec![0.7, 0.8, 0.9]]);
