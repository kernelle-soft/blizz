use jerrod::platform::{GitPlatform, Repository, MergeRequest, Discussion, User, ReactionType, MergeRequestState, Note, FileDiff, Pipeline, PipelineStatus};
...
impl MockGitHub {
    ...
    pub fn with_test_data() -> Self {
        ...
        // Add test MR
        let created_at = DateTime::parse_from_rfc3339("2023-01-01T00:00:00Z").unwrap().with_timezone(&Utc);
        let updated_at = DateTime::parse_from_rfc3339("2023-01-01T12:00:00Z").unwrap().with_timezone(&Utc);
        ...
        // Add test note
        let note_created_at = DateTime::parse_from_rfc3339("2023-01-01T10:00:00Z").unwrap().with_timezone(&Utc);
        let note = Note {
...
impl GitPlatform for MockGitHub {
    ...
    async fn get_merge_request(&self, owner: &str, repo: &str, number: u64) -> Result<MergeRequest> {
        ...
        self.merge_requests
            .get(&key)
            .cloned()
            .ok_or_else(|| anyhow::anyhow!("Merge request not found"))
    }

    async fn get_discussions(&self, _owner: &str, _repo: &str, number: u64) -> Result<Vec<Discussion>> {
        if self.should_fail {
            return Err(anyhow::anyhow!("Mock failure"));
        }
    ...
    async fn add_comment(&self, _owner: &str, _repo: &str, _discussion_id: &str, text: &str) -> Result<Note> {
        if self.should_fail {
            return Err(anyhow::anyhow!("Mock failure"));
        }
    ...
    async fn add_reaction(&self, _owner: &str, _repo: &str, _comment_id: &str, _reaction: ReactionType) -> Result<bool> {
        if self.should_fail {
            return Err(anyhow::anyhow!("Mock failure"));
        }
    ...
    async fn remove_reaction(&self, _owner: &str, _repo: &str, _comment_id: &str, _reaction: ReactionType) -> Result<bool> {
        if self.should_fail {
            return Err(anyhow::anyhow!("Mock failure"));
        }
    ...
    async fn get_reactions(&self, _owner: &str, _repo: &str, _comment_id: &str) -> Result<Vec<ReactionType>> {
        if self.should_fail {
            return Err(anyhow::anyhow!("Mock failure"));
        }
