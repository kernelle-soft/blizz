...
async fn test_github_platform_comments() {
    ...
    // Test add_comment
    let comment_result = mock.add_comment("test_owner", "test_repo", "thread_1", "Test comment").await;
...
async fn test_github_platform_reactions() {
    ...
    // Test add_reaction
    let reaction_result = mock.add_reaction("test_owner", "test_repo", "thread_1", ReactionType::ThumbsUp).await;
