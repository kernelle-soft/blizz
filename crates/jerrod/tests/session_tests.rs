...
fn create_test_discussion(id: &str, file_path: Option<String>, line_number: Option<u32>) -> Discussion {
    let note = Note {
        id: format!("note_{}", id),
        author: create_test_user(),
...
fn test_load_current_session_with_no_session() {
    ...
    assert!(error_msg.contains("session") || error_msg.contains("No") || error_msg.contains("not found"));
}
