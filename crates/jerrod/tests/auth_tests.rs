...
async fn test_get_github_token_with_missing_credentials() {
    ...
    assert!(error_msg.contains("credential") || error_msg.contains("token") || error_msg.contains("github"));
}
...
async fn test_get_gitlab_token_with_missing_credentials() {
    ...
    // Error should be related to missing credentials
    let error_msg = result.unwrap_err().to_string();
    assert!(error_msg.contains("credential") || error_msg.contains("token") || error_msg.contains("gitlab"));
}
