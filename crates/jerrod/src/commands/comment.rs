...
pub async fn handle(text: String, new: bool) -> Result<()> {
  ...
  if new {
    ...
    let _note = platform.add_comment(repo_parts[0], repo_parts[1], &pr_number, &text).await?;
    bentley::success("Added new comment to MR");
  } else if let Some(thread_id) = &current_thread_id {
    ...
    if let Some(thread) = original_thread {
      if thread.file_path.is_some() {
        ...
        let original_content = thread.notes.first().map(|note| &note.body).unwrap_or(&empty_string);
        ...
        let pr_number = session.merge_request.number.to_string();
        let _note =
          platform.add_comment(repo_parts[0], repo_parts[1], &pr_number, &referenced_text).await?;
