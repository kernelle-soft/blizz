...
pub async fn handle() -> Result<()> {
    ...
    println!("Branches: {} → {}", session.merge_request.source_branch, session.merge_request.target_branch);
    ...
    bentley::info(&format!("Session created: {}", session.created_at.format("%Y-%m-%d %H:%M:%S UTC")));
