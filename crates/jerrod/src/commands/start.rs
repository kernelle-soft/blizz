...
use crate::platform::{GitPlatform, detection::{detect_platform, PlatformType}};
use crate::platform::github::GitHubPlatform;
use crate::session::{ReviewSession, SessionManager};

pub async fn handle(repository: String, mr_number: u64, platform_override: Option<String>) -> Result<()> {
    ...
    let merge_request = platform.get_merge_request(&repo_info.owner, &repo_info.repo, mr_number).await?;
    ...
    bentley::success(&format!(
        ...
        session.merge_request.title,
        session.merge_request.state
    ));
