  ...
  match platform.resolve_discussion(owner, repo, &current_thread.id).await {
    ...
    Ok(false) => {
      ...
      match platform.add_reaction(owner, repo, &current_thread.id, ReactionType::ThumbsUp).await {
        ...
        Ok(false) => {
          bentley::warn(&format!(
            "Could not resolve thread #{} - reaction failed",
            current_thread.id
          ));
        }
        Err(reaction_err) => {
          bentley::error(&format!(
            ...
            current_thread.id, reaction_err
          ));
          return Err(anyhow!(
            "Resolution failed for thread #{}: {}",
            current_thread.id,
            reaction_err
          ));
        }
    ...
    Err(e) => {
      ...
      match platform.add_reaction(owner, repo, &current_thread.id, ReactionType::ThumbsUp).await {
        ...
        Ok(false) => {
          bentley::warn(&format!(
            "Could not resolve thread #{} - both native and reaction resolution failed",
            current_thread.id
          ));
        }
        Err(reaction_err) => {
          bentley::error(&format!(
            "Both resolution methods failed for thread #{}: native={}, reaction={}",
            current_thread.id, e, reaction_err
          ));
          return Err(anyhow!(
            "Both resolution and reaction failed for thread #{}",
            current_thread.id
          ));
        }
      }
