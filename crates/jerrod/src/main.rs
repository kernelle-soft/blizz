...
#[derive(Parser)]
#[command(name = "jerrod")]
#[command(about = "GitLab and GitHub merge request review tool - the reliable guardian of code quality")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Peek => {
            commands::peek::handle().await
        }
        Commands::Pop { unresolved } => {
            commands::pop::handle(unresolved).await
        }
        Commands::Comment { text, new } => {
            commands::comment::handle(text, new).await
        }
        Commands::Resolve => {
            commands::resolve::handle().await
        }
        Commands::Finish => {
            commands::finish::handle().await
        }
        Commands::Refresh => {
            commands::refresh::handle().await
        }
    }
} 